...
// --- Icon Definitions (Simplified for single-file) ---
const SearchIcon = () => (
...
// --- Navigation Sub-Menu Data (omitted for brevity) ---
const COURSES_MENU = [
...
// --- Dropdown Menu Component (omitted for brevity) ---
...
// --- Navbar Component (omitted for brevity) ---
        ...
        <header className="sticky top-0 z-20 bg-white border-b border-gray-100 shadow-md">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
                ...
                <nav className="hidden lg:flex items-center space-x-6">
                    ...
                    <button onClick={() => onViewChange('VERIFICATION_REQUEST')} className="text-gray-700 hover:text-gray-900 px-3 py-2 rounded-lg transition duration-150 text-base">Verify Account (Demo)</button>
                </nav>
...
// --- Dummy Components (Hero, Features, Categories, Testimonials, Footer) Omitted for brevity, kept structure ---
const HeroSection = () => <div className="p-16 text-center bg-blue-50/50">Hero Section</div>;
const FeaturesSection = () => <div className="p-16 text-center bg-white border-t">Features Section</div>;
const CategoriesSection = () => <div className="p-16 text-center bg-gray-50 border-t">Categories Section</div>;
const TestimonialsSection = () => <div className="p-16 text-center bg-white border-t">Testimonials Section</div>;
const Footer = () => <div className="p-10 text-center bg-gray-800 text-white">Footer</div>;


// Utility function to get auth token (MUST BE REPLACED)
const getAuthToken = () => {
    // !!! IMPORTANT: REPLACE THIS WITH YOUR ACTUAL TOKEN RETRIEVAL LOGIC !!!
    // e.g., return localStorage.getItem('userToken'); 
...
// --- Component 1: Account Verification Request (Step 1) ---
const AccountVerificationRequest = ({ onSuccess, onReturnToLanding }) => {
    ...
    const handleSendCode = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Transition to the next component after a brief delay
                setTimeout(() => onSuccess(email), 1000); 
            } else {
                // Error cases (HTTP 400, 404, etc.)
                setIsSuccess(false);
...
// --- Component 2: Account Verification Code Submission (Step 2) ---
const AccountVerificationCodeSubmission = ({ email, onReturnToRequest, onVerificationSuccess }) => {
    ...
    // !!! IMPORTANT: REPLACE THIS WITH YOUR ACTUAL RENDER HOSTED ENDPOINT !!!
    const VERIFICATION_SUBMISSION_ENDPOINT = 'https://your-hosted-backend.com/api/v1/users/verifyAccount'; 

    // Handle input change and move focus
    const handleInputChange = (e, index) => {
        const value = e.target.value.slice(-1); // Only take the last character entered
    ...
    const handleFormSubmit = async (e) => {
        ...
        const token = getAuthToken();
        if (token === "YOUR_ACTUAL_JWT_TOKEN") {
            setMessage("Error: Authentication token is missing or invalid. Please check the `getAuthToken` function.");
            setIsLoading(false);
            return;
        }

        try {
            const response = await fetch(VERIFICATION_SUBMISSION_ENDPOINT, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`, 
                ...
                body: JSON.stringify({ 
                    email, 
                    verificationCode // This maps to the codeValue in your controller
                }),
            });
            ...
            if (response.ok) {
                // Success case (HTTP 200)
                setIsSuccess(true);
                setMessage(data.message || "Account successfully verified! You are all set.");
                // Execute success callback (e.g., redirect user)
                setTimeout(onVerificationSuccess, 2000); 
            } else {
                // Error cases (HTTP 400 - Invalid/Expired Code, 404 - User not found, etc.)
                setIsSuccess(false);
                // Check if backend message exists, otherwise provide generic error
                setMessage(data.message || `Verification failed. Status: ${response.status}. Please check the code or request a new one.`);
            }
            ...
            <div className="w-full max-w-lg bg-white p-8 sm:p-10 rounded-xl shadow-2xl border border-gray-200">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-6">
                        A **6-digit code** was sent to: <span className="font-semibold" style={{ color: PRIMARY_BLUE }}>{email}</span>
                    </p>
                </div>
                    ...
                    <div className="flex justify-center space-x-2 sm:space-x-3">
                        {code.map((digit, index) => (
                            <input
                                ...
                                className="w-10 h-14 sm:w-12 sm:h-16 text-2xl font-bold text-center border-2 border-gray-300 rounded-lg shadow-sm focus:border-transparent focus:ring-4 focus:ring-opacity-50 transition duration-150"
                ...
                <div className="mt-8 text-center">
                    <button 
                        onClick={onReturnToRequest} 
                        className="text-sm font-medium transition duration-150 underline" 
                        style={{ color: PRIMARY_BLUE }}
                        disabled={isLoading || isSuccess}
                    >
                        Didn't receive a code or it expired? Resend code
                    </button>
                </div>
            </div>
        </div>
...
export default function IndexPage() {
    // State to manage the current step of the verification process (or landing page)
    const [currentView, setCurrentView] = useState('VERIFICATION_REQUEST'); 
    // State to hold the email once it's successfully submitted in the request step
    const [emailForVerification, setEmailForVerification] = useState('');
    
    const handleViewChange = (view) => {
        setCurrentView(view);
        // Reset email when navigating away from verification flow
        if (view === 'LANDING') {
    ...
    const handleCodeRequestSuccess = (email) => {
        setEmailForVerification(email);
        setCurrentView('VERIFICATION_SUBMISSION');
    };

    const handleVerificationSuccess = () => {
        // Here you would typically handle the final success, e.g., redirect to dashboard
        alert('SUCCESS! Account Verified. Redirecting to your dashboard...');
        handleViewChange('LANDING'); // For demo, return to landing page
    };

    const renderMainContent = () => {
        if (currentView === 'VERIFICATION_REQUEST') {
            return (
        ...
        if (currentView === 'VERIFICATION_SUBMISSION') {
            // Ensure we have an email before rendering submission component
            if (!emailForVerification) {
                return (
                    <div className="p-20 text-center">
                        <p className="text-xl text-red-500">Error: Missing email address. Please return to step 1.</p>
                        <button onClick={() => handleViewChange('VERIFICATION_REQUEST')} className="mt-4 text-blue-600 underline">
                            Go to Request Code
                        </button>
                    </div>
                );
            ...
            return (
                <AccountVerificationCodeSubmission 
                    email={emailForVerification}
                    onReturnToRequest={() => handleViewChange('VERIFICATION_REQUEST')}
            ...
            <>
                ...
                <div className="py-16 bg-white border-t border-gray-100">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                        ...
                        <div className="mt-8">
                            <button onClick={() => handleViewChange('VERIFICATION_REQUEST')} className="text-sm font-medium transition duration-150 underline" style={{ color: PRIMARY_BLUE }}>
